...
import { deals, contacts, tasks, notes, calls, accounts } from "../../lib/db/schema.js";
...
export function startReconciliationWorker(): Worker {
  return new Worker<ReconciliationJobData>(
    ...
    async (job) => {
      ...
      // Reconcile deals
      let dealsCount = 0;
      for await (const page of client.paginate("/leads", "leads", {
      ...
      // Reconcile contacts
      let contactsCount = 0;
      for await (const page of client.paginate("/contacts", "contacts", {
        "filter[updated_at][from]": since,
      })) {
        for (const c of page) {
          await db
      ...
      // Reconcile tasks
      let tasksCount = 0;
      for await (const page of client.paginate("/tasks", "tasks", {
        "filter[updated_at][from]": since,
      })) {
        for (const t of page) {
          await db
      ...
      console.log(
        ...
        `deals=${dealsCount} contacts=${contactsCount} tasks=${tasksCount}`
      );
    },
    {
      connection: redis,
